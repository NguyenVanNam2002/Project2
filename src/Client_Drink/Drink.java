...
public class Drink {
    ...
    public static ObservableList<Drink> selectAll() throws SQLException {
        ...
        try (
                ...
                ResultSet result = stsm.executeQuery("SELECT p.Name,p.Price FROM products as p join category as c on p.CategoryID = c.CategoryID WHERE c.CategoryID = 1");) {
    ...
    public static ObservableList selectImgByID() throws SQLException {
        ObservableList drink = FXCollections.observableArrayList();
        String sql = "SELECT p.ImgLink FROM products as p join category as c on p.Category = c.Category WHERE c.CategoryID = 1";
        try (
                Connection connect = DB_Connect.getConnection();
                Statement stsm = connect.createStatement();
                ResultSet result = stsm.executeQuery("SELECT p.ImgLink FROM products as p join category as c on p.CategoryID = c.CategoryID WHERE c.CategoryID = 1");) {
            while (result.next()) {
                drink.add(result.getString("ImgLink"));
            }
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
